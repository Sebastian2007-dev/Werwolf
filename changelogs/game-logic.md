...
## [2026-07-12 00:00] Auto-Modus: Karten-Bestätigung vor der ersten Nacht
- Im Auto-Modus (ohne Erzähler) startet die erste Nacht nicht mehr nach festem Timer, sondern erst, wenn **alle menschlichen Spieler** bestätigt haben, dass sie ihre Rollenkarte gesehen haben. Bots gelten automatisch als bereit. Danach läuft der gewohnte 5s-Countdown „Nacht beginnt in".
- Server: neues Spielzustandsfeld `cardConfirms` (Set), Helfer `pendingCardConfirms()`/`pushCardConfirmState()`, neues Socket-Event `confirm-card-seen`, Broadcast `card-confirm-update` mit der Liste der noch ausstehenden Spieler.
- Robustheit: Bestätigungsstand überlebt Reconnects (`replacePlayerSocket` + Nachreichen in `pushCurrentGameState`); verlässt ein Spieler das Spiel während der Wartephase, wird nicht ewig auf ihn gewartet (`handleGameLeave`). Verlässt der Erzähler das Spiel noch vor der ersten Nacht, greift die Bestätigungs-Runde ebenfalls statt des blinden Countdowns.
- Client (game.html/game.js/game.css): neues Panel „Karte gesehen ✓" unter den Karten-Aktionen. Der Knopf wird erst freigeschaltet, nachdem die Karte mindestens einmal aufgedeckt wurde; danach zeigt das Panel „Warten auf: …" mit den noch fehlenden Spielern. Cache-Versionen auf v=6 erhöht.
- Per Socket.IO-E2E-Test verifiziert (3 Menschen + 1 Bot): Bot nicht in der Warteliste, Countdown erst nach der letzten Bestätigung, Nacht startet danach normal.

## [2026-07-12 00:00] Gendarm an den Tag verlegt, Zigeunerin-Rache-Fluch, Dieb-Karten aus dem Pool
- **Gendarm (komplett umgebaut):** handelt nicht mehr nachts, sondern **am Tag vor der Abstimmung**. Nach der Anklage-Phase sehen alle eine neutrale Pause (verrät den Gendarm nicht); nur der Gendarm bekommt eine 5s-Abfrage „Willst du eingreifen?". Zückt er die Waffe, wird er **allen Spielern enthüllt** (Stern-Animation), wählt dann sein Ziel (30s), und die Karte des Opfers wird öffentlich aufgedeckt. Ist das Opfer unschuldig, richtet er sich selbst. Neue Phasen `gendarm-window`/`gendarm-shoot` inkl. Erzähler-Skip, Bot-Verhalten (schießt nur auf sicher bekannte Wölfe), Reconnect-Support und Disconnect-Behandlung.
- **Zigeunerin (Regel korrigiert):** stirbt weiterhin sofort bei Anklage, aber ihr Fluch ist jetzt eine **öffentliche Rache am Tag**: alle sehen, wen sie verflucht — die verfluchte Person **verwandelt sich in einen Werwolf** (zählt ab sofort für Siegbedingung, Wolfsrunde und Rudel-Chat; Bots merken es sich als sicheres Wolfswissen). Der alte heimliche Nacht-Fluch („tötet einen Werwolf") wurde entfernt. Neue Phase `zigeunerin-curse` mit Fluch-Overlay; die tote Zigeunerin wählt ihr Ziel selbst (45s, Bots: Top-Verdächtiger).
- **Dieb:** seine 2 Wahlkarten stammen jetzt aus dem **echten Kartenpool** statt aus beliebigen unbenutzten Rollen. Ist der Dieb ausgewählt, wird der Pool auf Spielerzahl+2 mit Dorfbewohnern aufgefüllt; die nach dem Austeilen übrigen 2 Karten sind seine Optionen. Werwolf-Karten landen nie in den Dieb-Karten (gehen immer an Spieler), die Dieb-Karte selbst wird garantiert ausgeteilt.
- **Jack the Ripper:** nur Beschreibungstexte korrigiert (roles.js, roles.md) — er hat keine eigene Nachtwahl; er tötet die Dorfmatratze nur, wenn sie bei IHM schläft, und mutiert dann. Die Spiellogik war bereits korrekt.
- **Silberschmied:** per E2E-Test verifiziert, dass die Mechanik funktioniert (Wölfe greifen die ausgerüstete Person an → ein zufälliger Wolf stirbt, die Person überlebt, Morgentext erscheint). Kein Codeänderungsbedarf.
- Frontend: neue Overlays für Gendarm (Abfrage mit Countdown, Enthüllung, Karten-Aufdeckung) und Zigeunerin-Fluch (Zielwahl als Tote, öffentliches Verwandlungs-Ergebnis, „Das bist du"-Hinweis für die verfluchte Person), neue Phasen-Labels für Zuschauer und Erzähler.
- Alles per Socket.IO-E2E-Suite getestet: Silberschmied-Schutz, kompletter Gendarm-Tag-Flow (inkl. „kein Nachtzug mehr"), Zigeunerin-Fluch bis zum Wolfssieg durch Verwandlung, Dieb-Optionen ohne Wölfe.

## [2026-07-12 00:00] Einkaufs-Anzeige, strengere Anklage-Regel, Spielbericht am Spielende
- **Händler/Einkaufen sichtbar:** Während Anklage- und Abstimmungsphase sehen jetzt alle Spieler einen Hinweis „🛒 X ist einkaufen und setzt diese Runde aus" im Tag-Panel (Server sendet `awayPlayerName` mit, auch nach Reconnect).
- **Anklage-Regel verschärft:** Die Abstimmung entfällt nur noch, wenn **alle** Spieler überspringen. Vorher reichte eine Überspringer-Mehrheit — jetzt führt schon eine einzige Anklage zur Abstimmung.
- **Spielbericht (Chronik):** Neues Feld `chronicle` sammelt den kompletten Spielverlauf mit Namen — pro Nacht alle Aktionen (Silberschmied-Ziel, Wolfsopfer, Seherin-Blicke, Hexen-Tränke, Dorfmatratzen-Schlafplatz, …) plus Tote des Morgens, pro Tag Anklagen, Lynch-Ergebnis (mit Rolle), Stichwahl, Gendarm-Eingriff, Zigeunerin-Fluch und Jäger-Schüsse. Auf dem Game-Over-Screen öffnet der neue Knopf „📜 Spielbericht anzeigen" ein scrollbares Modal, gruppiert nach „Nacht 1 / Tag 1 / …".
- **Auflösungs-Notizen ergänzt:** Wen der Silberschmied ausgerüstet hat, welche zwei Karten der Dieb zur Wahl hatte und ob er Dieb blieb, stehen jetzt zusätzlich direkt in der Auflösung unter dem Rollen-Grid.
- Per Socket.IO-E2E-Tests verifiziert: 1 Anklage + 2 Überspringer → Abstimmung findet statt; alle überspringen → kein Vote; Chronik enthält Nacht-/Tag-Abschnitte mit Namen; `awayPlayerName` kommt bei allen an.
